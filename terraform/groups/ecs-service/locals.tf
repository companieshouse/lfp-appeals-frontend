...
locals {
  stack_name                = "company_requests" # this must match the stack name the service deploys into
  ...
  task_secrets = [
    { "name": "ACCOUNT_URL", "valueFrom": "${local.secrets_arn_map.account_url}" },
    { "name": "CACHE_SERVER", "valueFrom": "${local.secrets_arn_map.cache_server}" },
    { "name": "COOKIE_SECRET", "valueFrom": "${local.secrets_arn_map.cookie_secret}" },
    { "name": "FILE_TRANSFER_API_KEY", "valueFrom": "${local.secrets_arn_map.file_transfer_api_key}" },
    { "name": "OAUTH2_CLIENT_ID", "valueFrom": "${local.secrets_arn_map.oauth2_client_id}" },
    { "name": "OAUTH2_CLIENT_SECRET", "valueFrom": "${local.secrets_arn_map.oauth2_client_secret}" },
    { "name": "OAUTH2_REQUEST_KEY", "valueFrom": "${local.secrets_arn_map.oauth2_request_key}" },
    { "name": "OAUTH2_TOKEN_URI", "valueFrom": "${local.secrets_arn_map.oauth2_token_uri}" },
  ]
