...
@controller(WHO_WAS_ILL_PAGE_URI, FeatureToggleMiddleware(Feature.ILLNESS_REASON),
    SessionMiddleware, AuthMiddleware)
export class WhoWasIllController extends BaseController<FormBody> {
    ...
    protected prepareSessionModelPriorSave(appeal: Appeal, value: any): Appeal {
        ...
        loggerInstance()
            .debug(`${WhoWasIllController.name} - prepareSessionModelPriorSave: ${JSON.stringify(appeal)}`);
        return appeal;
    }
}
