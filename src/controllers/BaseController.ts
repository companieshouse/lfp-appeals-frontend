...
export type ChangeModeAction = (req: Request, step: keyof Navigation, defaultAction: string) => string;

const createChangeModeAwareNavigationProxy = (step: Navigation, changeModeAction: ChangeModeAction): Navigation => {
    return new Proxy(step, {
        get(target: Navigation, propertyName: keyof Navigation): any {
            return (req: Request) => {
                if (req.query.cm === '1') {
                    return changeModeAction(req, propertyName, CHECK_YOUR_APPEAL_PAGE_URI);
                }
